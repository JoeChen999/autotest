...
class Device:
    ...
    def get_screen_info(self):
        ...
        assert s is not None
        self.resolution_width = float(s.groups()[0])
        self.resolution_height = float(s.groups()[1])
        s = re.search(r"(\d+)\s*x\s*(\d+)\s*\(real\s*screen\)", res)
        assert s is not None
        self.real_width = float(s.groups()[0])
        self.real_height = float(s.groups()[1])
    ...
    def check_saved_pic(self, name, fault=True):
        res = checkpic.saved_pic_check(self.__device, self.running_case_name, 0.9, self.check_num, name, fault)
        if fault:
            self.check_num += 1
        return res

    def check_pic(self, x, y, width, height, fault=True):
        res = checkpic.pic_check(self.__device, self.running_case_name, 0.9, self.check_num, x, y, width, height, fault)
        if fault:
            self.check_num += 1
        return res
